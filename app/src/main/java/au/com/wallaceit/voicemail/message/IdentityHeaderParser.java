...
public class IdentityHeaderParser {
    ...
    public static Map<IdentityField, String> parse(final String identityString) {
        ...
        if (VisualVoicemail.DEBUG) {
            Log.d(VisualVoicemail.LOG_TAG, "Decoding identity: " + identityString);
        }
        ...
        if (identityString.charAt(0) == IdentityField.IDENTITY_VERSION_1.charAt(0) && identityString.length() > 2) {
            ...
            if (VisualVoicemail.DEBUG) {
                Log.d(VisualVoicemail.LOG_TAG, "Decoded identity: " + identity.toString());
            }
            ...
            for (IdentityField key : IdentityField.getIntegerFields()) {
                if (identity.get(key) != null) {
                    try {
                        ...
                        Log.e(VisualVoicemail.LOG_TAG, "Invalid " + key.name() + " field in identity: " + identity.get(key));
                    }
                }
            }
        } else {
            ...
            if (VisualVoicemail.DEBUG) {
                Log.d(VisualVoicemail.LOG_TAG, "Got a saved legacy identity: " + identityString);
            }
            ...
            if (tokenizer.hasMoreTokens()) {
                ...
                try {
                    ...
                    Log.e(VisualVoicemail.LOG_TAG, "Unable to parse bodyLength '" + bodyLengthS + "'");
                }
            }
            if (tokenizer.hasMoreTokens()) {
