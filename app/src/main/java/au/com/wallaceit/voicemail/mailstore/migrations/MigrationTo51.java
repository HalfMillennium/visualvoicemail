...
class MigrationTo51 {
    ...
    public static void db51MigrateMessageFormat(SQLiteDatabase db, MigrationsHelper migrationsHelper) {
        ...
        attachmentDirNew = StorageManager.getInstance(VisualVoicemail.app).getAttachmentDirectory(
        ...
        try {
            Log.d(VisualVoicemail.LOG_TAG, "migrating " + msgCursor.getCount() + " messages");
            ...
            while (msgCursor.moveToNext()) {
                ...
                try {
                    ...
                    Log.e(VisualVoicemail.LOG_TAG, "error inserting into database", e);
                }
            }
    ...
    private static File renameOldAttachmentDirAndCreateNew(Account account, File attachmentDirNew) {
        ...
        if (!moveOk) {
            // TODO escalate?
            Log.e(VisualVoicemail.LOG_TAG, "Error moving attachment dir! All attachments might be lost!");
        }
        boolean mkdirOk = attachmentDirNew.mkdir();
        if (!mkdirOk) {
            // TODO escalate?
            Log.e(VisualVoicemail.LOG_TAG, "Error creating new attachment dir!");
        }
        return attachmentDirOld;
    }

    private static void dropOldMessagesTable(SQLiteDatabase db) {
        Log.d(VisualVoicemail.LOG_TAG, "Migration succeeded, dropping old tables.");
    ...
    private static void cleanUpOldAttachmentDirectory(File attachmentDirOld) {
        for (File file : attachmentDirOld.listFiles()) {
            Log.d(VisualVoicemail.LOG_TAG, "deleting stale attachment file: " + file.getName());
            if (file.exists() && !file.delete()) {
                Log.d(VisualVoicemail.LOG_TAG, "Failed to delete stale attachement file: " + file.getAbsolutePath());
            }
        }

        Log.d(VisualVoicemail.LOG_TAG, "deleting old attachment directory");
        if (attachmentDirOld.exists() && !attachmentDirOld.delete()) {
            Log.d(VisualVoicemail.LOG_TAG, "Failed to delete old attachement directory: " + attachmentDirOld.getAbsolutePath());
        }
    }
    ...
    private static MimeStructureState migratePgpMimeEncryptedContent(SQLiteDatabase db, long messageId,
        ...
        Log.d(VisualVoicemail.LOG_TAG, "Attempting to migrate multipart/encrypted as pgp/mime");
        ...
        try {
            if (cursor.getCount() != 2) {
                Log.e(VisualVoicemail.LOG_TAG, "Found multipart/encrypted but bad number of attachments, handling as regular mail");
                return null;
            }
    ...
    private static MimeStructureState migrateComplexMailContent(SQLiteDatabase db,
        ...
        Log.d(VisualVoicemail.LOG_TAG, "Processing mail with complex data structure as multipart/mixed");
    ...
    private static MimeStructureState migrateSimpleMailContent(SQLiteDatabase db, String htmlContent,
            ...
            throws IOException {
        Log.d(VisualVoicemail.LOG_TAG, "Processing mail with simple structure");
    ...
    private static MimeStructureState insertMimeAttachmentPart(SQLiteDatabase db, File attachmentDirOld,
        ...
        if (VisualVoicemail.DEBUG) {
            Log.d(VisualVoicemail.LOG_TAG, "processing attachment " + id + ", " + name + ", "
        ...
        if (hasData) {
            try {
                ...
                if (!isMatchingAttachmentId) {
                    Log.e(VisualVoicemail.LOG_TAG, "mismatched attachment id. mark as missing");
                    attachmentFileToMove = null;
                } else if (!isExistingAttachmentFile) {
                    Log.e(VisualVoicemail.LOG_TAG, "attached file doesn't exist. mark as missing");
                    attachmentFileToMove = null;
                } else {
        ...
        if (VisualVoicemail.DEBUG && attachmentFileToMove == null) {
            Log.d(VisualVoicemail.LOG_TAG, "matching attachment is in local cache");
        }
        ...
        if (attachmentFileToMove != null) {
            ...
            if (!moveOk) {
                Log.e(VisualVoicemail.LOG_TAG, "Moving attachment to new dir failed!");
            }
        }
        return structureState;
    }
