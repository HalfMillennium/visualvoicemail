...
public class NotificationActionService extends CoreService {
    ...
    public int startService(Intent intent, int startId) {
        ...
        if (ACTION_MARK_AS_READ.equals(action)) {
            ...
            if (VisualVoicemail.DEBUG) {
                Log.i(VisualVoicemail.LOG_TAG, "Notification dismissed");
            }
        }
    ...
    private void archiveMessages(Intent intent, Account account, MessagingController controller) {
        ...
        if (archiveFolderName == null ||
                (archiveFolderName.equals(account.getSpamFolderName()) && VisualVoicemail.confirmSpam()) ||
                !isMovePossible(controller, account, archiveFolderName)) {
            Log.w(VisualVoicemail.LOG_TAG, "Can not archive messages");
            return;
        }
    ...
    private void markMessageAsSpam(Intent intent, Account account, MessagingController controller) {
        ...
        if (spamFolderName != null && !VisualVoicemail.confirmSpam() &&
    ...
    private boolean isMovePossible(MessagingController controller, Account account,
        ...
        boolean isSpecialFolderConfigured = !VisualVoicemail.FOLDER_NONE.equalsIgnoreCase(destinationFolderName);
