package au.com.wallaceit.voicemail.activity.loader;
    ...
    public Attachment loadInBackground() {
        ...
        if (size <= 0) {
            ...
            if (uriString.startsWith("file://")) {
                Log.v(VisualVoicemail.LOG_TAG, uriString.substring("file://".length()));
                File f = new File(uriString.substring("file://".length()));
                size = f.length();
            } else {
                Log.v(VisualVoicemail.LOG_TAG, "Not a file: " + uriString);
            }
        } else {
            Log.v(VisualVoicemail.LOG_TAG, "old attachment.size: " + size);
        }
        Log.v(VisualVoicemail.LOG_TAG, "new attachment.size: " + size);
