...
class StoreSchemaDefinition implements LockableDatabase.SchemaDefinition {
    ...
    public void doDbUpgrade(final SQLiteDatabase db) {
        try {
            ...
            Log.e(VisualVoicemail.LOG_TAG, "Exception while upgrading database. Resetting the DB to v0", e);
            db.setVersion(0);
            upgradeDatabase(db);
        }
    }
    ...
    private void upgradeDatabase(final SQLiteDatabase db) {
        Log.i(VisualVoicemail.LOG_TAG, String.format(Locale.US, "Upgrading database from version %d to version %d",
            ...
            if (db.getVersion() < 29) {
                    ...
                    try {
                        ...
                        while (cursor.moveToNext()) {
                            try {
                                ...
                                Log.e(VisualVoicemail.LOG_TAG, " error trying to ugpgrade a folder class", e);
                            }
                        }
                    } catch (SQLiteException e) {
                        Log.e(VisualVoicemail.LOG_TAG, "Exception while upgrading database to v41. folder classes may have vanished", e);
                    } finally {
                ...
                if (db.getVersion() == 41) {
                    try {
                        ...
                        Log.i(VisualVoicemail.LOG_TAG, "Putting folder preferences for " + folders.size() + " folders back into Preferences took " + (endTime - startTime) + " ms");
                    } catch (Exception e) {
    ...
    private void update41Metadata(final SQLiteDatabase  db, SharedPreferences prefs, int id, String name) {
        ...
        try {
            ...
            Log.e(VisualVoicemail.LOG_TAG, " Throwing away an error while trying to upgrade folder metadata", e);
        }
