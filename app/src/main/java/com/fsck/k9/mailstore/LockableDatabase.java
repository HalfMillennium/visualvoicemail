...
public class LockableDatabase {
    ...
    private class StorageListener implements StorageManager.StorageListener {
        ...
        public void onUnmount(final String providerId) {
            ...
            if (VisualVoicemail.DEBUG) {
                Log.d(VisualVoicemail.LOG_TAG, "LockableDatabase: Closing DB " + uUid + " due to unmount event on StorageProvider: " + providerId);
            }
        ...
        public void onMount(final String providerId) {
            ...
            if (VisualVoicemail.DEBUG) {
                Log.d(VisualVoicemail.LOG_TAG, "LockableDatabase: Opening DB " + uUid + " due to mount event on StorageProvider: " + providerId);
            }
    ...
    public <T> T execute(final boolean transactional, final DbCallback<T> callback) throws MessagingException {
        ...
        try {
            final boolean debug = VisualVoicemail.DEBUG;
            if (doTransaction) {
            ...
            try {
                ...
                if (doTransaction) {
                    ...
                    mDb.endTransaction();
                    if (debug) {
                        Log.v(VisualVoicemail.LOG_TAG, "LockableDatabase: Transaction ended, took " + Long.toString(System.currentTimeMillis() - begin) + "ms / " + new Exception().getStackTrace()[1].toString());
                    }
                }
            }
        } finally {
    ...
    public void switchProvider(final String newProviderId) throws MessagingException {
        if (newProviderId.equals(mStorageProviderId)) {
            Log.v(VisualVoicemail.LOG_TAG, "LockableDatabase: Ignoring provider switch request as they are equal: " + newProviderId);
            return;
        }
        ...
        try {
            ...
            try {
                try {
                    ...
                    Log.i(VisualVoicemail.LOG_TAG, "Unable to close DB on local store migration", e);
                }
    ...
    private void delete(final boolean recreate) throws UnavailableStorageException {
        ...
        try {
            try {
                ...
                if (VisualVoicemail.DEBUG)
                    Log.d(VisualVoicemail.LOG_TAG, "Exception caught in DB close: " + e.getMessage());
            }
            final StorageManager storageManager = getStorageManager();
            try {
                ...
                if (VisualVoicemail.DEBUG)
                    Log.d(VisualVoicemail.LOG_TAG, "Exception caught in clearing attachments: " + e.getMessage());
            }
            try {
                deleteDatabase(storageManager.getDatabase(uUid, mStorageProviderId));
            } catch (Exception e) {
                Log.i(VisualVoicemail.LOG_TAG, "LockableDatabase: delete(): Unable to delete backing DB file", e);
            }
