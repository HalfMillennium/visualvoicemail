...
public class MessageCryptoHelper {
        ...
        new OpenPgpServiceConnection(context, openPgpProvider,
                new OnBound() {
                    ...
                    public void onError(Exception e) {
                        Log.e(VisualVoicemail.LOG_TAG, "Couldn't connect to OpenPgpService", e);
                    }
                }).bindToService();
    }
    ...
    private void decryptVerify(Intent intent) {
        ...
        try {
            ...
            Log.e(VisualVoicemail.LOG_TAG, "IOException", e);
        } catch (MessagingException e) {
            Log.e(VisualVoicemail.LOG_TAG, "MessagingException", e);
        }
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.d(VisualVoicemail.LOG_TAG, "signed data type: " + signatureBodyPart.getMimeType());
                    signatureBodyPart.writeTo(out);
                } catch (Exception e) {
                    Log.e(VisualVoicemail.LOG_TAG, "Exception while writing message to crypto provider", e);
                } finally {
                    try {
                        out.close();
                    } catch (IOException e) {
            ...
            public void run() {
                try {
                    ...
                    if (cryptoPartType == CryptoPartType.ENCRYPTED) {
                        ...
                        Log.wtf(VisualVoicemail.LOG_TAG, "No suitable data to stream found!");
                    }
                } catch (Exception e) {
                    Log.e(VisualVoicemail.LOG_TAG, "Exception while writing message to crypto provider", e);
                } finally {
        ...
        new AsyncTask<Void, Void, MimeBodyPart>() {
            ...
            protected MimeBodyPart doInBackground(Void... params) {
                ...
                try {
                    ...
                    Log.w(VisualVoicemail.LOG_TAG, "we were interrupted while waiting for onReturn!", e);
                } catch (Exception e) {
                    Log.e(VisualVoicemail.LOG_TAG, "Something went wrong while parsing the decrypted MIME part", e);
    ...
    private void onCryptoOperationReturned(MimeBodyPart outputPart) {
        if (currentCryptoResult == null) {
            Log.e(VisualVoicemail.LOG_TAG, "Internal error: we should have a result here!");
            return;
        }
    ...
    private void handleCryptoOperationResult(MimeBodyPart outputPart) {
        ...
        if (VisualVoicemail.DEBUG) {
            Log.d(VisualVoicemail.LOG_TAG, "OpenPGP API decryptVerify result code: " + resultCode);
        }

        switch (resultCode) {
            case INVALID_OPENPGP_RESULT_CODE: {
                Log.e(VisualVoicemail.LOG_TAG, "Internal error: no result code!");
                break;
            }
    ...
    private void handleUserInteractionRequest() {
        ...
        try {
            ...
            Log.e(VisualVoicemail.LOG_TAG, "Internal error on starting pendingintent!", e);
        }
    }
    ...
    private void handleCryptoOperationError() {
        ...
        if (VisualVoicemail.DEBUG) {
            Log.w(VisualVoicemail.LOG_TAG, "OpenPGP API error: " + error.getMessage());
        }
