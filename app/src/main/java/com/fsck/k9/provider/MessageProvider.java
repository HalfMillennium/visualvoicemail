...
public class MessageProvider extends ContentProvider {
            ...
            for (Account account : Preferences.getPreferences(getContext()).getAvailableAccounts()) {
                if (account.getAccountNumber() == accountNumber) {
                    ...
                    try {
                        ...
                        Log.e(VisualVoicemail.LOG_TAG, e.getMessage());
                        values[0] = "Unknown";
                        values[1] = 0;
                    }
                }
    ...
    protected static class MonitoredCursor implements CrossProcessCursor {
        ...
        public void close() {
            if (mClosed.compareAndSet(false, true)) {
                mCursor.close();
                Log.d(VisualVoicemail.LOG_TAG, "Cursor closed, null'ing & releasing semaphore");
                mCursor = null;
                mSemaphore.release();
            }
        }
    ...
    protected class ThrottlingQueryHandler implements QueryHandler {
        ...
        public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
            ...
            if (!(cursor instanceof CrossProcessCursor)) {
                Log.w(VisualVoicemail.LOG_TAG, "Unsupported cursor, returning null: " + cursor);
                mSemaphore.release();
                return null;
            }
            ...
            mScheduledPool.schedule(new Runnable() {
                ...
                public void run() {
                    ...
                    if (monitored != null && !monitored.isClosed()) {
                        Log.w(VisualVoicemail.LOG_TAG, "Forcibly closing remotely exposed cursor");
                        try {
                            monitored.close();
                        } catch (Exception e) {
                            Log.w(VisualVoicemail.LOG_TAG, "Exception while forcibly closing cursor", e);
                        }
                    }
                }
            }, 30, TimeUnit.SECONDS);
    ...
    protected class MesssageInfoHolderRetrieverListener extends MessagingListener {
        ...
        public void searchStats(AccountStats stats) {
            try {
                ...
                Log.e(VisualVoicemail.LOG_TAG, "Unable to return message list back to caller", e);
            }
        }
    }
    ...
    public boolean onCreate() {
        ...
        VisualVoicemail.registerApplicationAware(new VisualVoicemail.ApplicationAware() {
            @Override
            public void initializeComponent(final Application application) {
                Log.v(VisualVoicemail.LOG_TAG, "Registering content resolver notifier");
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        if (VisualVoicemail.app == null) {
            return 0;
        }

        if (VisualVoicemail.DEBUG) {
            Log.v(VisualVoicemail.LOG_TAG, "MessageProvider/delete: " + uri);
        }
        ...
        for (Account account : Preferences.getPreferences(getContext()).getAccounts()) {
            if (account.getAccountNumber() == accountId) {
                ...
                if (!account.isAvailable(getContext())) {
                    Log.w(VisualVoicemail.LOG_TAG, "not deleting messages because account is unavailable at the moment");
                    return 0;
                }
            }
        }
        ...
        if (myAccount == null) {
            Log.e(VisualVoicemail.LOG_TAG, "Could not find account with id " + accountId);
        }
        ...
        try {
            ...
            if (VisualVoicemail.DEBUG) {
                Log.d(VisualVoicemail.LOG_TAG, "folder msg count = " + msgCount);
            }
            msg = lf.getMessage(msgUid);
        } catch (MessagingException e) {
            Log.e(VisualVoicemail.LOG_TAG, "Unable to retrieve message", e);
        }
    ...
    public String getType(Uri uri) {
        ...
        if (VisualVoicemail.DEBUG) {
            Log.v(VisualVoicemail.LOG_TAG, "MessageProvider/getType: " + uri);
        }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (VisualVoicemail.DEBUG) {
            Log.v(VisualVoicemail.LOG_TAG, "MessageProvider/insert: " + uri);
        }
    ...
    public Cursor query(final Uri uri, final String[] projection, final String selection,
        ...
        if (VisualVoicemail.DEBUG) {
            Log.v(VisualVoicemail.LOG_TAG, "MessageProvider/query: " + uri);
        }
        ...
        try {
            ...
            Log.e(VisualVoicemail.LOG_TAG, "Unable to execute query for URI: " + uri, e);
            return null;
        }
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (VisualVoicemail.DEBUG) {
            Log.v(VisualVoicemail.LOG_TAG, "MessageProvider/update: " + uri);
        }
